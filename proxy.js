...
// Fonction de connexion HTTP pour obtenir les cookies
...
// Connexion WebSocket avec les cookies récupérés
function connectPO() {
...
(async () => {
  if (!PO_EMAIL || !PO_PASSWORD) {
    console.error('❌ PO_EMAIL et PO_PASSWORD doivent être définis dans les variables d\'environnement Render');
    return;
  }
  const ok = await loginAndGetSession();
  if (ok) {
    connectPO();
    // Rafraîchir la session toutes les 5 heures
    setInterval(async () => {
